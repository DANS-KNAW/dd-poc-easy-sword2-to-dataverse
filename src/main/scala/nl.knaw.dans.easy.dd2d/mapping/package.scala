...
package object mapping {
  val yyyyddmmPattern: DateTimeFormatter = DateTimeFormat.forPattern("YYYY-MM-dd")
  ...
  def hasXsiType(node: Node, xsiType: String): Boolean = {
    node.attribute("http://www.w3.org/2001/XMLSchema-instance", "type").map(_.text).map(t => t.endsWith(s":$xsiType") || t == xsiType).exists(identity)
  }
