...
class DepositSpec extends TestSupportFixture {
  ...
  it should "fail if it is not a directory but a file" in {
    ...
    the[CorruptDepositException] thrownBy Deposit(file) should have message (s"Not a deposit: $file is not a directory")
  }

  it should "fail if it has no sub-directoires" in {
    val file = testDirNonValid / "no-subdir"
    the[CorruptDepositException] thrownBy Deposit(file) should have message (s"Not a deposit: $file has more or fewer than one subdirectory")
  }

  it should "fail if it has no deposit.properties" in {
    val file = testDirNonValid / "no-deposit-properties"
    the[CorruptDepositException] thrownBy Deposit(file) should have message (s"Not a deposit: $file does not contain a deposit.properties file")
  }
